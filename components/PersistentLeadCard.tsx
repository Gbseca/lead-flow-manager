...
export const PersistentLeadCard: React.FC<PersistentLeadCardProps> = ({ lead, onUpdate, onRemove, onOpenWhatsApp }) => {
  ...
  const handleSave = () => {
    onUpdate(lead.wa, { note, scheduleISO: schedule, priority });
  };
  ...
  const isOverdue = countdown === 'Atrasado';

  return (
    <div className={`rounded-lg p-4 border ${isOverdue ? 'border-[var(--danger-text)] bg-[var(--danger)]/10' : 'border-[var(--warning-text)]/50 bg-[var(--warning)]/10'} hover:border-[var(--warning-text)] transition-colors animate-card-entry`}>
      <div className="flex items-start gap-4">
        <div className="flex-shrink-0 w-12 h-12 rounded-full bg-[var(--bg-tertiary)] flex items-center justify-center font-bold text-xl text-[var(--warning-text)]">{initials}</div>
          ...
          <div className="flex justify-between items-start">
            ...
            <div className="flex items-center gap-2 flex-shrink-0 ml-2">
               <a href={lead.tel} className="p-2 rounded-full bg-[var(--bg-secondary)] hover:bg-[var(--success)] text-[var(--text-secondary)] hover:text-[var(--accent-text)] transition-colors"><PhoneIcon className="w-5 h-5" /></a>
                ...
                <button onClick={() => onRemove(lead.wa)} className="p-2 rounded-full bg-[var(--bg-secondary)] hover:bg-[var(--warning)] text-[var(--warning-text)] hover:text-[var(--accent-text)] transition-colors">
          ...
          <div>
            ...
            <div className="relative">
              <input
                ...
                className="w-full mt-1 bg-[var(--bg-secondary)] border border-[var(--border-primary)] rounded-md p-2 text-sm text-[var(--text-primary)] focus:ring-1 focus:ring-[var(--warning)] focus:border-[var(--warning)]"
              />
              {countdown && <span className={`absolute right-2 top-1/2 -translate-y-1/2 mt-0.5 text-xs font-bold ${isOverdue ? 'text-[var(--danger-text)]' : 'text-[var(--text-secondary)]'} bg-[var(--bg-tertiary)] px-2 py-0.5 rounded`}>{countdown}</span>}
            </div>
          </div>
          <div>
      ...
      <div className="mt-4 flex justify-end gap-2">
        <button onClick={handleSnooze} className="px-4 py-2 text-sm font-semibold rounded-md bg-[var(--warning)]/20 hover:bg-[var(--warning)]/40 text-[var(--warning-text)] transition-colors">Adiar 15min</button>
